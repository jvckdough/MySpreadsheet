...
public abstract class AbstractSpreadsheet
{
    ...
    ///
    /// If the cell referred to by name is involved in a circular dependency,
    /// throws a CircularException. This is detected by checking if name has
    /// a dependent of itself.
    /// 
    /// Otherwise, visits each dependent of name checking if their values need to be updated.
    /// Recursion is used to visit each dependent and its dependents, adding each dependent to a hashset called 'visited'
    /// and if they need to be updated it is added to the list 'changed'. 
    /// </summary>
